...
using DKit.Modules.LanguageLocalizationHelper.Runtime;
using DKit.Modules.LanguageLocalizationHelper.Runtime.Core;
using UnityEditor;
using UnityEngine;

namespace DKit.Modules.LanguageLocalizationHelper.Editor
{
    [CustomEditor(typeof(LLImage))]
    public class LLImageEditor : UnityEditor.Editor
    {
        private LLImage LT;
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField(LT.tip.ToString(), this[12, FontStyle.Normal, TextAnchor.MiddleCenter, Color.white]);
            #region 筛选器

            //筛选
            EditorGUILayout.LabelField("列表关键字搜索");
            ...
            if (isFoldout)
            {
                scrollRect = EditorGUILayout.BeginScrollView(scrollRect, this[LayoutStyle.GroupBox]);
                {
                    ...
                    {
                        if (data.llObjects[i].llType == LLType.Image)
                        {
