...
using Modules.protobuf_unity.Editor;


namespace Im.Unity.protobuf_unity.Editor
{
    public class ProtobufCompiler : AssetPostprocessor
    {
        ...
        {
            get { return Directory.GetFiles(Application.dataPath, "*.proto", SearchOption.AllDirectories); }
        }
        ...
        {
            get { return AllProtos.Select(t => Path.GetDirectoryName(t)).Distinct(); }
        }
        ...
        private static string BinaryPath
        {
            get
            {
                if (string.IsNullOrEmpty(BinaryPathCache))
                {
                    ...
                    {
                        BinaryPathCache = Path.Combine(Directory.GetParent(Application.dataPath).FullName, libraryPath,
                            BinaryRelativePath);
                    }
                }
...
# elif UNITY_EDITOR_WIN
                string protocPath = Path.Combine(BinaryPath,"protoc");
                if (!File.Exists(protocPath))
                {
                    protocPath =
 Path.Combine(Directory.GetParent(GetCurSourceFileAbsDir).FullName, "Binary", "Win", "protoc");
                }
                return protocPath;
#else
        ...
        /// <param name="protobufConfig">Target configuration</param>
        public static bool Compile(string absolutePath, ProtobufConfig protobufConfig)
        {
            if (protobufConfig == null) return false;
            ...
            if (!aPath.StartsWith(vPath))
            {
                ...
                {
                    Debug.LogError(ProtobufCompilerLog + "Cannot compile " + absolutePath +
                                   " because isn't inside Assets folder");
                    return false;
                }
            ...
            if (!absolutePath.EndsWith(".proto"))
            {
                ...
                {
                    Debug.LogError(ProtobufCompilerLog + "Cannot compile " + absolutePath +
                                   " because isn't a protobuf file (.proto)");
                    return false;
                }
            ...
            {
                var absoluteOutput = string.IsNullOrEmpty(config.TargetLocation)
                    ? parentFolder
                    : RelativeToAbsolute(config.TargetLocation);
                ...
                var compilationArguments =
                    string.Format(config.CompilerFormat, absoluteOutput) + includes + compilingFile;
                if (ProtobufPreferences.LogDebug)
                {
        ...
        /// </summary>
        /// <param name="protoFilePath">Full path to the .proto</param>
        /// <returns></returns>
